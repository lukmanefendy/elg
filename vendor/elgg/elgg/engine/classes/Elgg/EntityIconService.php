...
class EntityIconService {
	...
	//ditambahkan:save from uploaded file
	public function saveFromUploadedFile(ElggEntity $entity, $input_name, $type = 'icon') {
		$files = $this->request->files;
		if (!$files->has($input_name)) {
			return false;
		}
		...
		$result = $this->saveFile($entity, $tmp, $type);
	...
	//ditambahkan:save file uploaded
	public function saveFile(ElggEntity $entity, ElggFile $file, $type = 'icon') {
		...
		if (!$file instanceof ElggFile || !$file->exists() || $file->getSimpleType() !== 'image') {
			...
			$icon = $this->getIcon($entity, array(), $type);
