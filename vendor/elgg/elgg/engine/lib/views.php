...
function elgg_view_page($title, $body, $page_shell = 'default', $vars = array()) {
	...
	//ditambahkan:front_page untuk jenis front page
	if ($_GET['__elgg_uri'] == '/rekomen')
		$page_shell = 'rekomen';
	else if ($_GET['__elgg_uri'] == '/kriteria')
		$page_shell = 'kriteria';
	else if ($_GET['__elgg_uri'] == '/area')
		$page_shell = 'area';
	else if ($_GET['__elgg_uri'] == '/cara')
		$page_shell = 'cara';
	...
	//ditambahkan jenis front page
	/* if ($_GET['__elgg_uri'] == '/')
		$output = elgg_view("page/$page_shell", $vars);
	if ($_GET['__elgg_uri'] == '/rekomen')
		$output = elgg_view("page/rekomen", $vars);
	 */
	$output = elgg_view("page/$page_shell", $vars);
...
function elgg_views_boot() {
	...
	if (!isset($CONFIG->icon_sizes)) {
		$icon_sizes = array(
			...
			//'vsmall' => array('w' => 550, 'h' => 550, 'square' => false, 'upscale' => false),
			//'vmedium' => array('w' => 100, 'h' => 100, 'square' => true, 'upscale' => true),
			//'vlarge' => array('w' => 200, 'h' => 200, 'square' => false, 'upscale' => false),
			//'vmaster' => array('w' => 550, 'h' => 550, 'square' => false, 'upscale' => false),
		);
