...
function elgg_format_element($tag_name, array $attributes = array(), $text = '', array $options = array()) {
	...
	//ditambahkan:hidden utk public access
	$hide = $attributes['name'];
	if ($hide == "accesslevel[catexpert]" || $hide == "accesslevel[areaexpert]" || 
		$hide == "accesslevel[hrm]" || $hide == "accesslevel[pubadmin]" || $hide == "accesslevel[declaration]"
		|| $hide == "accesslevel[check]")
		$attrs = " hidden".$attrs;
