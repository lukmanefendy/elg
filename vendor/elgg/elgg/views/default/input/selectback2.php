...
if (isset($vars['name']) && $vars['name'] == 'catexpert'){
	...
	$vars['options_values'] = 
		...
		<label for="profile-areaexpert" class="elgg-field-label-child">Area of expertise</label>
...
/* else if (isset($vars['name']) && $vars['name'] == 'areaexpert'){
	$vars['class'] = elgg_extract_class($vars, 'areaexpert hidden');
	$vars['options'] = 
		array(
			'0' => '',
			'Human resources management' => 'Human resources management',
...
if (isset($vars['name']) && $vars['name'] == 'catexpert'){	
	...
	$(document).ready(function() {
		$('.catexpert').change(function(){
			if($('.catexpert').val() == "Both Categories"){
				$('.academic').show();
				$('.practioners').show();
			}
			else if($('.catexpert').val() == "Practitioners"){
				$('.academic').hide();
				$('.practioners').show();
			}
			else if($('.catexpert').val() == "Academic Expert"){
				$('.academic').show();
				$('.practioners').hide();
			}
			if($('.catexpert').val() == 0){
