...
//ditambahkan:sql untuk cek user_entity sdh yes or no
$isvrf = get_data("SELECT vrf from elgg_users_entity where guid=$user->guid");
/* if ($isvrf[0]->vrf == 'yes' && $size == 'large')
	$vrf=elgg_get_simplecache_url('verified.png');
else if ($isvrf[0]->vrf == 'yes' && $size == 'small')
	$vrf=elgg_get_simplecache_url('vsmall.png');
else $vrf=''; */
